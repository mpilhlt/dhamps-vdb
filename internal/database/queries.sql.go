...
func (q *Queries) GetProjectsByUser(ctx context.Context, arg GetProjectsByUserParams) ([]GetProjectsByUserRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&i.Owner, &i.ProjectHandle, &i.Role); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
type GetSharedUsersForDefinitionRow struct {
	UserHandle string `db:"user_handle" json:"user_handle"`
	Role       string `db:"role" json:"role"`
}

func (q *Queries) GetSharedUsersForDefinition(ctx context.Context, arg GetSharedUsersForDefinitionParams) ([]GetSharedUsersForDefinitionRow, error) {
	rows, err := q.db.Query(ctx, getSharedUsersForDefinition, arg.Owner, arg.DefinitionHandle)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetSharedUsersForDefinitionRow
	for rows.Next() {
		var i GetSharedUsersForDefinitionRow
...
func (q *Queries) LinkDefinitionToUser(ctx context.Context, arg LinkDefinitionToUserParams) error {
	_, err := q.db.Exec(ctx, linkDefinitionToUser, arg.UserHandle, arg.DefinitionID, arg.Role)
	return err
}
...
SELECT definition_id, definition_handle, owner, endpoint, description, api_standard, model, dimensions, context_limit, created_at, updated_at
FROM definitions
WHERE "owner" = $1
AND "definition_handle" = $2
LIMIT 1
